...
IDX_LI_BINDING = PROPERTY_KEYS.index("li_binding_energy_kJ_mol")
...
D_SOLV = 4             # per-species solvation context features (P_shell, binding_centered/kT, pair_drive/kT, cage_contrib)
...
def init_params(key: jax.Array, mix_mean: np.ndarray, mix_std: np.ndarray) -> dict:
    ...
    d_enc_in = D_INPUT + 3 + D_SOLV  # props + [log_frac, frac, T_scaled] + solvation context
...
def _compute_per_species_solvation(raw_props, fracs, mask, T_K):
    """Per-species solvation context: (N_max, D_SOLV).

    Each species gets features describing its role in the Li⁺ solvation shell.
    Goes through attention (per-species), NOT the global readout — so new species
    are handled by the permutation-invariant attention mechanism, not a fixed MLP.
    """
    lambda0 = raw_props[:, IDX_LAMBDA0]
    ionic_indicator = jnp.where(jnp.abs(lambda0) > 0.1, 1.0, 0.0)
    solvent_indicator = 1.0 - ionic_indicator

    li_bind = raw_props[:, IDX_LI_BINDING]
    K_coord = raw_props[:, IDX_COORD_AFFINITY]

    Kc = K_coord * fracs * mask * solvent_indicator
    Kc_sum = jnp.maximum(Kc.sum(), 1e-8)
    P_shell = Kc / Kc_sum

    shell_bind_mean = jnp.sum(P_shell * li_bind)

    kT_kJ = K_B * T_K * N_A / 1000.0

    feat_P_shell = P_shell
    feat_binding_centered = (li_bind - shell_bind_mean) * solvent_indicator / jnp.maximum(kT_kJ, 1e-3)
    ion_pair_bind = raw_props[:, IDX_ION_PAIR_BINDING]
    feat_pair_drive = (ion_pair_bind - shell_bind_mean) * ionic_indicator / jnp.maximum(kT_kJ, 1e-3)
    total_cage_energy = jnp.maximum(shell_bind_mean, 1e-8)
    feat_cage_contrib = P_shell * li_bind / total_cage_energy

    solv_ctx = jnp.stack([feat_P_shell, feat_binding_centered, feat_pair_drive, feat_cage_contrib], axis=-1)
    return solv_ctx * mask[:, None]
...
def forward_single(params, species_props, raw_props, fracs, mask, temperature_K,
    ...
    solv_ctx = _compute_per_species_solvation(raw_props, fracs, mask, temperature_K)
...
def main():
    ...
    model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mol_set_sigma_solvshell.pkl")
